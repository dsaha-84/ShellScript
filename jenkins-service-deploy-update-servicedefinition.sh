#!/usr/bin/env bash
#==================================================================================================================
# Script: jenkins-service-deploy-update-servicedefinition.sh
# Description:
#		This script will perform a rolling upgrade for a service with the latest docker image.
#       It takes the latest image url as input and update the service with latest image
#==================================================================================================================
...
if [ $SERVICE != false ]; then
  ...
  #The cloudformation script create the service name with some convention
  SERVICE_NAME=`$AWS_ECS list-services --cluster $CLUSTER | jq -r .serviceArns | grep -i $SERVICE | cut -d \/ -f 2 | sed -e 's/"//g'`
  if [[ ! -z ${SERVICE_NAME} ]]; then
    TASK_DEFINITION=`$AWS_ECS describe-services --services $SERVICE_NAME --cluster $CLUSTER | jq -r .services[0].taskDefinition`
  else
      echo "Could not find any service with the given service name"
      exit 1
  fi
fi
...
if [ $SERVICE == false ]; then
  ...
  # Update the service
  UPDATE=`$AWS_ECS update-service --cluster $CLUSTER --service $SERVICE_NAME $DESIRED_COUNT --task-definition $NEW_TASKDEF $DEPLOYMENT_CONFIG`

  # Only excepts RUNNING state from services whose desired-count > 0
  SERVICE_DESIREDCOUNT=`$AWS_ECS describe-services --cluster $CLUSTER --service $SERVICE_NAME | jq '.services[]|.desiredCount'`
  if [ $SERVICE_DESIREDCOUNT -gt 0 ]; then
    ...
    do
      ...
      RUNNING_TASKS=$($AWS_ECS list-tasks --cluster "$CLUSTER"  --service-name "$SERVICE_NAME" --desired-status RUNNING \
          | jq -r '.taskArns[]')
